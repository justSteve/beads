...
## beads — Enterprise Identity

This repo is **beads**: a **zepo** (enterprise infrastructure fork) in Steve's Zgent Enterprise.

- **Role:** Task tracking
  substrate — every zgent's bd commands flow through this
- **Type:** Zepo — infrastructure fork with active upstream intelligence
- **Bead prefix:** `be`
- **Upstream:** `gastownhall/beads` (fetch freely, never push enterprise artifacts to it)
- **Origin:** `justSteve/beads`

### Fork Doctrine

This fork exists because Steve deliberately appropriated the intellectual capital
of `gastownhall/beads`. This is offensive, not defensive — Steve chose to stand on these
